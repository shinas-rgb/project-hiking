...
export default function SearchBar() {
  ...
  return (
    <div className='flex justify-center max-sm:text-xs'>
      <div className='bg-zinc-800 flex items-center mt-4 border border-zinc-700 text-gray-300 py-1.5 px-3 rounded-xl gap-1'>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <input type="text" placeholder="Search for places" className="w-fit text-base"
            {...register("search")} />
        </form>
        <p className="text-xs rounded-full px-1.5 bg-zinc-700 text-zinc-500">x</p>
      </div>
    </div >
  )
}
